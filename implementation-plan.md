...
- Vertex AI service-account authentication enables Gemini explanation; model selection uses `VERTEX_AI_MODEL_ID`, then legacy `GEMINI_MODEL`. Without valid Google credentials/project configuration, or when the call fails, the product returns a deterministic verified summary.
